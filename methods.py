...
def setup_driver() -> webdriver.Firefox:
    if not check_permission(environment.url, environment.USER_AGENT): 
        sys.exit("Robots.txt disallows crawling this URL.")
        
    service = Service(executable_path=environment.EXECUTABLE_PATH) 
    options = Options()
    options.add_argument("--headless")
    options.binary_location = environment.BINARY_PATH 
...
def reach_website(driver: webdriver.Firefox) -> None:
    driver.get(environment.url)   # reaches the website
...
def check_password_strength(driver) -> None:
    ...
    pwd_input.clear()
    pwd_input.send_keys(environment.PASSWORD)
    ...
    try:
        score = get_zxcvbn_score(driver, environment.PASSWORD)
